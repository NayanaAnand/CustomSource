...
PREPARE_LOGGING(CustomSource_i)

CustomSource_i::CustomSource_i(const char *uuid, const char *label) :
        CustomSource_base(uuid, label), server_(NULL), client_(NULL), sendNewSri(
                true) {
    ...
    addPropertyChangeListener("byte_swap", this,
            &CustomSource_i::byte_swapChanged);
    addPropertyChangeListener("connection_type", this,
            &CustomSource_i::connection_typeChanged);
    addPropertyChangeListener("ip_address", this,
            &CustomSource_i::ip_addressChanged);
    addPropertyChangeListener("max_bytes", this,
            &CustomSource_i::max_bytesChanged);
    addPropertyChangeListener("min_bytes", this,
            &CustomSource_i::min_bytesChanged);
    addPropertyChangeListener("port", this, &CustomSource_i::portChanged);
    addPropertyChangeListener("sri", this, &CustomSource_i::sriChanged);
    addPropertyChangeListener("internal_buffer_size", this,
            &CustomSource_i::internal_buf_szChanged);
    addPropertyChangeListener("socket_settings", this,
            &CustomSource_i::socket_settingsChanged);
...
void CustomSource_i::start() throw (CF::Resource::StartError, CORBA::SystemException){
    updateXferLen();
    updateSocket(true);
    CustomSource_base::start();

}

CustomSource_i::~CustomSource_i() {
...
void CustomSource_i::byte_swapChanged(const unsigned short *oldValue,
...
void CustomSource_i::connection_typeChanged(const std::string *oldValue,
...
void CustomSource_i::ip_addressChanged(const std::string *oldValue,
        const std::string *newValue) {
    if (*oldValue != *newValue) {
        if (started()) {
            boost::recursive_mutex::scoped_lock lock(socketLock_);
...
void CustomSource_i::max_bytesChanged(const CORBA::ULong *oldValue,
...
void CustomSource_i::min_bytesChanged(const CORBA::ULong *oldValue,
        const CORBA::ULong *newValue) {
    if (*oldValue != *newValue) {
        boost::recursive_mutex::scoped_lock lock(xferLock_);
        updateXferLen();
...
void CustomSource_i::portChanged(const unsigned short *oldValue,
        const unsigned short *newValue) {
    if (*oldValue != *newValue) {
        if (*newValue < 1024) {
...
void CustomSource_i::internal_buf_szChanged(const CORBA::ULong *oldValue,
        const CORBA::ULong *newValue) {
    if (*oldValue != *newValue) {
        if (started()) {
            boost::recursive_mutex::scoped_lock lock(socketLock_);
...
void CustomSource_i::socket_settingsChanged(const socket_settings_struct *oldValue,
...
void CustomSource_i::sriChanged(const sri_struct *oldValue,
...
int CustomSource_i::serviceFunction() {
    ...
    size_t numRead = 0;
    LOG_INFO(CustomSource_i, __PRETTY_FUNCTION__ << " before read: maxBytes=" << maxBytes
    ...
    try {
        ...
        LOG_INFO(CustomSource_i, "Data Received at Source and data is : " << data_[data_length_]);
        ...
        LOG_ERROR(CustomSource_i, "Exception caught: " << e.what());
        throw(e);
    }
    LOG_INFO(CustomSource_i,
            "Received " << numRead<< " bytes, max size = " << maxBytes);
    LOG_INFO(CustomSource_i, __PRETTY_FUNCTION__ << " after read: maxBytes=" << maxBytes
                                          << " sock_read_buf_.size()=" << data_.size()
                                          << " data_length_=" << data_length_
                                          << " data_capacity_=" << data_capacity_);
    ...
    if (data_length_ >= minBytes || (numRead == 0 && data_length_ >= multSize && status == "not connected")) {
        ...
        if (activePorts_.size() > 1) {
            ...
            for (std::vector<std::string>::iterator i = activePorts_.begin();
                ...
                warnstr += *i + " ";
            LOG_WARN(CustomSource_i, warnstr + ".");
        }
        LOG_INFO(CustomSource_i, __PRETTY_FUNCTION__ << " after push: maxBytes=" << maxBytes
                                              << " sock_read_buf_.size()=" << data_.size()
                                              << " data_length_=" << data_length_
                                              << " data_capacity_=" << data_capacity_);
        return NORMAL;
    }
    LOG_INFO(CustomSource_i, __PRETTY_FUNCTION__ << " NO push: maxBytes=" << maxBytes
                                          << " sock_read_buf_.size()=" << data_.size()
                                          << " data_length_=" << data_length_
                                          << " data_capacity_=" << data_capacity_
                                          << " minBytes=" << minBytes
...
void CustomSource_i::updateXferLen() {
...
void CustomSource_i::updateSocket(bool starting) {
    ...
    if (connection_type == "server" && port > 0) {
        try {
            ...
            LOG_ERROR(CustomSource_i,
                    "Exception starting server on port " << port << ": " << e.what());
        }
        LOG_INFO(CustomSource_i, "Set as SERVER :" << port);
        ...
        try {
            ...
            LOG_ERROR(CustomSource_i, "Exception starting client: " << e.what());
        }
    } else {
        LOG_ERROR(CustomSource_i,
